...
class DividendTriggerTest {
	...
	private GameContext contextWithHoldings(int tick, PlayerId playerId, Map<Symbol, Integer> holdings) {
		...
		return new GameContext(tick, players, Map.of(AAPL, INITIAL_PRICE, GOOG, INITIAL_PRICE), List.of());
	}

	private GameContext contextWithTwoPlayers(int tick, Map<Symbol, Integer> p1Holdings,
			Map<Symbol, Integer> p2Holdings) {
		Map<PlayerId, GameEvent.PlayerPortfolio> players = new HashMap<>();
	...
	void noAwardOnNonIntervalTick() {
		...
		List<AwardedPowerup> awards = trigger.evaluate(contextWithHoldings(5, PLAYER_1, Map.of(AAPL, 15)));
		assertTrue(awards.isEmpty());
	}

	@Test
	void noAwardAtTickZero() {
		List<AwardedPowerup> awards = trigger.evaluate(contextWithHoldings(0, PLAYER_1, Map.of(AAPL, 15)));
		assertTrue(awards.isEmpty());
	}
	...
	void noAwardWithLessThanFiveShares() {
		...
		List<AwardedPowerup> awards = trigger.evaluate(contextWithHoldings(400, PLAYER_1, Map.of(AAPL, 4)));
		assertTrue(awards.isEmpty());
	}
	...
	void tier1AwardAfter300TickHoldWithFiveShares() {
		...
		List<AwardedPowerup> awards = trigger.evaluate(contextWithHoldings(300, PLAYER_1, Map.of(AAPL, 5)));
	...
	void tier2InstantWhenHoldTimeCarriesOver() {
		...
		List<AwardedPowerup> awards = trigger.evaluate(contextWithHoldings(100, PLAYER_1, Map.of(AAPL, 10)));
		assertEquals(1, awards.size());
		DividendPowerup powerup = (DividendPowerup) awards.get(0).powerup();
	...
	void tier3InstantWithFifteenShares() {
		...
		List<AwardedPowerup> awards = trigger.evaluate(contextWithHoldings(20, PLAYER_1, Map.of(AAPL, 15)));
		assertEquals(1, awards.size());
		DividendPowerup powerup = (DividendPowerup) awards.get(0).powerup();
	...
	void holdTimerResetsWhenDroppingBelowFiveShares() {
		...
		// At tick 300 (payout tick), hold duration from 202 is only 98 — not enough for tier 1 (needs 300)
	...
	void multipleSymbolsTrackedIndependently() {
		...
		List<AwardedPowerup> awards = trigger.evaluate(contextWithHoldings(20, PLAYER_1, Map.of(AAPL, 15, GOOG, 5)));
	...
	void multiplePlayersGetIndependentAwards() {
		...
		List<AwardedPowerup> awards = trigger
				.evaluate(contextWithTwoPlayers(20, Map.of(AAPL, 15), Map.of(AAPL, 15)));
	...
	void correctPayoutAmount() {
		...
		List<AwardedPowerup> awards = mainTrigger
				.evaluate(contextWithHoldings(300, PLAYER_1, Map.of(AAPL, 5)));
		assertEquals(1, awards.size());
		DividendPowerup powerup = (DividendPowerup) awards.get(0).powerup();
